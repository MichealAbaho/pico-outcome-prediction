...
def read_anns(phase, element, ann_type='aggregated', model_phase='train'):
  ...
  for fname in fnames:
    labels = [int(i) for i in open(fname).readlines()]
    d = os.path.basename(fname).split('.')
    pmid, wid = d[0], d[1]
    if pmid not in docs:
  ...
  print('Loaded annotations for %d documents from %d worker%s' % (
    len(docs), len(workers), 's' if len(workers) != 1 else ''))
  return workers, docs
...
def print_labeled_spans_2(doc):
  x = []
  for wid, labels in doc.anns.items():
      label_spans = condense_labels(labels)
      y = []
      for label, token_i, token_f in label_spans:
        y.append((doc.decoder[label], ' '.join(doc.tokens[token_i:token_f])))
      x.append(y)
