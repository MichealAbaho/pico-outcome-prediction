...
import os
import shutil
from ebm_nlp_1_00.base_models import data_prep
...
tf.flags.DEFINE_string("ebm_file", "/users/phd/micheala/Documents/Github/pico-back-up/ebm_nlp_1_00/labels_outcomes_2.csv", "Data source.")
...
def train(_xtr, _xval, _ytr, _yval,  classes, vocabularly, emb_matrix):
    ...
    mod = ls_model.fit(_xtr, _ytr, validation_data=(_xval, _yval), callbacks=callbacks, epochs=Flags.epochs, batch_size=Flags.batch_size)
...
def classification_metric(y_true, y_pred, classes):
    ...
    return classification_report(y, Y, target_names=classes)
...
def plotting_history(mod, fold):
    ...
    fig.savefig('lstm_cv_{}'.format(fold))
    #plt.show()

if __name__=='__main__':
    x, y, clas, voc, emb_matrix = load_vectorize_shuffle_data(cross_validation=True)
    mod =train_cross_fold(x[:200], y[:200], clas, voc, emb_matrix)
    plotting_history(mod)
